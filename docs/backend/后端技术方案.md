...
- 当前代码已在阶段 7 落地 `random_pick_sessions / random_pick_results` 及对应 API。
- `filter_snapshot` 用于回溯当时条件，防止 UI 条件变化后历史不可复现。
- `meal_plan_items.random_session_id` 用于把接受后的菜单项回溯到具体随机 session。
...
- 排除本周已加入的菜
- 排除当前 session 已跳过的菜
- 成员偏好标签预留位（当前作为加权匹配信号）

#### 单次抽取策略

1. 根据筛选条件查询候选池。
2. 过滤最近食用记录。
3. 先走严格匹配：分类、标签、难度同时满足。
4. 若严格匹配为空，则回落到“类似推荐”层，至少命中同分类、同标签或偏好标签之一。
5. 若用户未传分类和标签，允许再回落到仅满足难度限制的通用候选层。
6. 各层内部按权重随机抽取 1 个当前版本，并写入 `reason_meta`。
7. 若三层都为空，返回“请放宽条件”。

补充说明：

- 权重会综合考虑分类命中、标签命中、偏好标签命中、推断难度以及“快手/简单”标签。
- `accept` 支持可选 `recipeVersionId`，用于命中菜谱后的“换版本”。
- `single` 模式接受后 session 结束；跳过后 session 继续保持 `running`，可继续重抽。
...
- 尽量避免同分类连续扎堆。
- 同一菜谱一周内自动去重。
- 默认以 `weekStartDate` 对齐周一，未传时按当前日期推导所在周周一。
- 若可用菜谱不足 7 道，直接返回“候选菜谱不足以完成连抽，请放宽条件后再试”。
- `week` 模式接受时默认写入结果自带的 `pickedForDate`；全部结果都被接受或跳过后 session 才会转为 `completed`。
...
| 方法 | 路径 | 说明 |
| --- | --- | --- |
| `POST` | `/api/v1/random-picks/sessions` | 创建随机 session，支持 `single / week` 两种模式并返回结果 |
| `POST` | `/api/v1/random-picks/sessions/:id/redraw` | 仅 `single` 模式可用，复用同条件重抽一次 |
| `POST` | `/api/v1/random-picks/sessions/:id/results/:resultId/accept` | 接受结果并加入点菜台，支持可选 `recipeVersionId` 换版本 |
| `POST` | `/api/v1/random-picks/sessions/:id/results/:resultId/skip` | 跳过结果并更新 decision |
| `GET` | `/api/v1/random-picks/sessions/:id` | 查看本次抽取记录与完整结果历史 |
...
| `POST` | `/api/v1/shopping-lists/:id/share-image` | 生成分享图（阶段 6 先同步返回 SVG 数据，后续可切异步任务） |
...
| `random-pick.session.cleanup` | 定时任务 | 按需清理废弃 session，当前阶段 7 未启用 |
