...
本文档基于 [需求文档v2.md](D:\AI\Menu%20Time\需求文档v2.md) 输出“食光记”后端技术方案，作为后续后端开发、数据库设计、接口拆分、联调与部署的指导文档。
...
本文档以 `需求文档v2.md` 为主设计依据，同时参考 `TODO.md` 中对 MVP 的收敛建议。结论如下：
...
| 鉴权 | Auth.js（原 NextAuth） | MVP 支持单管理员登录，后续扩展家庭成员 |
...
| `width` | int | 宽度，可空 |
| `height` | int | 高度，可空 |
| `purpose` | varchar(30) | `moment / cover / share` |
| `created_by` | uuid | 上传人 |
| `created_at` | timestamptz | 创建时间 |
...
| 方法 | 路径 | 说明 |
| --- | --- | --- |
| `POST` | `/api/v1/media/upload-token` | 获取上传签名或临时凭证 |
| `POST` | `/api/v1/media/complete` | 上传完成后登记资源 |
...
```text
households/{householdId}/moments/{momentId}/{fileName}
households/{householdId}/recipes/{recipeId}/cover/{fileName}
households/{householdId}/shopping-lists/{listId}/share/{fileName}
```
...
- 所有 API 基于会话校验当前 `household_id`，不能只按资源 ID 查询。
...
1. 初始化 Next.js 项目、Prisma、PostgreSQL、Auth.js。
