...
这是一份面向后端开发的实施清单，基于 [需求文档v2.md](D:\AI\Menu%20Time\需求文档v2.md) 与 [后端技术方案.md](D:\AI\Menu%20Time\docs\后端技术方案.md) 输出，目标是把技术方案进一步拆成可执行、可验收、可排期的后端任务列表。
...
- 表结构、接口路径、事务规则以技术方案文档为准，本清单主要负责“做什么、先做什么、做到什么程度算完成”。
...
- [ ] 建立全局错误处理和业务异常包装。
- [ ] 接入 Auth.js，完成管理员登录、登出、当前会话的基础能力。
...
- [ ] 时光记录新增后维护 `recipes.latest_moment_at`、`latest_cooked_at`、`moment_count`。
...
- [ ] 实现 `shopping-list.share-image` 任务。
- [ ] 实现 `random-pick.session.cleanup` 定时清理任务。
- [ ] 预留统计刷新任务入口。
- [ ] 接入 Sentry 捕获后端错误。
- [ ] 对关键接口和任务输出结构化日志。
...
- 本文档不覆盖根目录 [TODO.md](D:\AI\Menu%20Time\TODO.md)，而是在 `docs` 目录下作为独立文档维护。
- 本文档覆盖 PRD 全量后端能力，但开发顺序仍以核心闭环优先。
- 具体表结构字段定义、API 路径和事务规则以 [后端技术方案.md](D:\AI\Menu%20Time\docs\后端技术方案.md) 为准。
